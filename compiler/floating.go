...
func fmadd(w *OutputWriter, command AssemblyCommand) {
	WriteIndentedString(w, "registers[\"%s\"] = %s * %s + %s\n", command.Arguments[0].Source, CompileRegister(w, command.Arguments[1]), CompileRegister(w, command.Arguments[2]), CompileRegister(w, command.Arguments[3]))
}
...
func fsgnj(w *OutputWriter, command AssemblyCommand) {
	WriteIndentedString(w, "registers[\"%s\"] = abs(%s) * sgn(%s)\n", command.Arguments[0].Source, CompileRegister(w, command.Arguments[1]), CompileRegister(w, command.Arguments[2]))
}
